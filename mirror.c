...
typedef struct {
	...
	sph_radiogatun64_context	radiogatun64;
	sph_panama_context	panama;
} mirrorhash_context_holder;

mirrorhash_context_holder base_contexts;

void init_mirrorhash_contexts()
{
  sph_sha256_init(&base_contexts.sha256);
  sph_radiogatun64_init(&base_contexts.radiogatun64);
  sph_panama_init(&base_contexts.panama);
 
}

static void mirrorhash(void *state, const void *input)
{
	mirrorhash_context_holder ctx;
    ...
    sph_radiogatun64 (&ctx.radiogatun64, input, 80);
    sph_radiogatun64_close (&ctx.radiogatun64, hashA);	
	
    sph_panama (&ctx.panama, hashA, 64);   
    sph_panama_close(&ctx.panama, hashB);  
	
    sph_sha256 (&ctx.sha256, hashB, 64);   
    sph_sha256_close(&ctx.sha256, hashA);  
    
    sph_sha256 (&ctx.sha256, hashA, 64);   
    sph_sha256_close(&ctx.sha256, hashB); 
	
	sph_panama (&ctx.panama, hashB, 64);   
    sph_panama_close(&ctx.panama, hashA); 
	
	sph_radiogatun64 (&ctx.radiogatun64, hashA, 64);   
    sph_radiogatun64_close(&ctx.radiogatun64, hashB); 
...
int scanhash_mirror(int thr_id, uint32_t *pdata, const uint32_t *ptarget,
